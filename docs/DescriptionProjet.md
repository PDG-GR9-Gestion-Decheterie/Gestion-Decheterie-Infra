...
Le but principal est de fournir un outil qui simplifiera la gestion des déchèteries nationales. Cette base de données permettra un suivi précis des informations essentielles, comme par exemple les ramassages, les employés, etc.
...
- Un employé possède un nom, prénom, une date de naissance, un numéro de téléphone, une adresse, date de début contrat, une fonction, un supérieur et un type de permis de conduire (B, C) facultatif. Chaque employé est affecté à une déchèterie et possède un login pour se connecter à l’application.
- Une adresse est décrite par une rue, un numéro, un code postal et une ville.
- Il y a comme fonctions : Responsable, Secrétaire, Chauffeur, Employé.

Une déchèterie peut posséder des moyens de transport des déchets (véhicules).

Les contenants sont soit des bennes, des palettes, des grandes caisses ou des big bag et possèdent un type de déchets contenu.
...
- Toutes les informations doivent être uniquement disponibles pour les employés de la même déchèterie principale. Par exemple il est impossible de voir les ramassages d'une autre déchèterie principale.
...
- On peut modifier le mot de passe via un responsable.

#### 1.2.4. Exigences non-fonctionnelles

- Un employé sans connaissances en informatique doit pouvoir prendre en main l’application en moins de 20 minutes avec l'aide d'une formation ou d'un mode d'emploi.
- Le système sera disponible 24 heures sur 24, avec un taux d'indisponibilité n'exédant pas 5 minutes par jour.
- Le système doit pouvoir gérer 1000 requêtes par seconde avec 90% du temps de réponse inférieur à 500ms.
...
- Les mots de passe sont stockés dans la base de données sous forme de hash.
...
Le reverse proxy utilise **Traefik** et permet au backend et au frontend d'être accessibles via la même url et le même port tout en redirigeant les différentes requêtes au bon conteneur.
...
**Accueil après connexion :** Une fois connecté, l'utilisateur revient sur la page d'accueil du logiciel. Cette page est la même que la page d'acceuil hors connexion mais avec des options de navigation, telles que "Employés", "Ramassages", "Véhicules", et "Déchetèries", chacune représentée par un bouton distinct. A savoir que ceci n'est qu'un exemple, les boutons pourront varier selon la personne connectée.
...
**Sélection de "Ramassage" :** L'utilisateur sélectionne l'option "Ramassages" pour accéder à la gestion des ramassages. Cela le redirige vers une page qui lui permet de voir la liste de ramassages concernant ses déchèteries.

**Détail d'un Ramassage :** En cliquant sur un ramassage spécifique dans la liste, l'utilisateur est dirigé vers une page de détails. Cette page affiche des informations complètes sur le ramassage sélectionné tel que l'état actuel du ramassage et d'autres informations critiques.
...
Nous avons également créé une [Landing page](https://pdg-gr9-gestion-decheterie.github.io/) que vous pouvez aller consulter à l'url suivante : https://pdg-gr9-gestion-decheterie.github.io

## 4. Description des choix techniques

![Infra](img/infra.png "Infrastructure")

Voici une liste des technologies que nous avons utilisé, ainsi qu'une explication de notre choix pour chacune :
...
Nous avons choisi **Node.js** pour son efficacité à gérer plusieurs clients simultanément sur un même thread, grâce à son architecture non bloquante et basée sur des événements. Cette approche permet de garder de bonnes performances, notamment dans le cadre d'une API où un grand nombre de requêtes concurrentes sont effectuées.
...
Après avoir comparé diverses méthodes Agiles, nous avons opté pour la méthode Extreme Programming (XP) afin de développer notre application. Cette décision s'appuie sur les avantages spécifiques de XP, qui incluent :
...
Nous avons également décidé de mettre en place les pratiques suivantes pour nous aider dans le développement :
...
Après avoir imaginé notre projet ainsi que toutes ses fonctionnalités, une équipe s'est attardée sur la réalisation des mockups/landing page et l'autre s'est occupée de la création de l'organisation Git ainsi que la mise en place du pipeline de développement.
Nous avons ensuite décidé de rester sur une double équipe de deux pour la réalisation du backend d'une part et du frontend de l'autre.
...
Comme mentionné au point précèdent, nous avons créé une organisation pour ce projet et nous y avons ajouté plusieurs repos répartis de manière claire :
...
Tous nos ajouts et modifications de code sont effectués sur des branches de développement (**dev**). Une fois que le travail sur une fonctionnalité (ou tâche) est terminé, nous effectuons un "merge request" pour intégrer ces nouveautés dans la branche **main**.
Une autre personne doit ensuite consulter ce merge request et l'approuver afin que le pipeline applique ces modifications automatiquement.

#### 5.4.2. Pipeline

Le Pipeline est composé de scripts github actions qui monitorent les pushs sur la branche **main** sur le repository du **backend** et du **frontend**. Quand une modification est faite sur une de ces branches, une image docker est automatiquement construite et push sur **Docker Hub** pour être accessible à l'environnement de déploiement. Le serveur de deploiment n'a plus qu'à pull les dernières images avec la commande `docker compose pull` et relancer le docker compose.
